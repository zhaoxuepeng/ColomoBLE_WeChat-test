 ...
 * @fn          i2cMstRepStrt
 *
 * @brief       Attempt to send an I2C bus Repeat START and Slave Address as an I2C bus Master.
 *
 * input parameters
 *
 * @param       RD_WRn - The LSB of the Slave Address as Read/~Write.
 ...
 * @return      The I2C status of the START request or of the Slave Address Ack.
 */
static uint8 i2cMstRepStrt(uint8 RD_WRn)
{
  // Vinci: It seems like a hardware bug, Work round here
  I2C_DISABLE();
  I2C_ENABLE();
  // ----------------------------------------------------
  I2C_STRT();
  
  if (I2CSTAT == mstStarted) /* A Repeat start condition has been transmitted */
  {
    I2C_WRITE(i2cAddr | RD_WRn);
  }
 ...
 * input parameters
 *
 * @param       subAddr - register address of slave device
 ...
 */
uint8 HalI2CReads(uint8 subAddr, uint8 len, uint8 *pBuf)
{
  uint8 cnt = 0;

  // Write ST, slave address and get ack from slave
  if (i2cMstStrt(0) != mstAddrAckW)
  {
    len = 0;
  }

  // Write subaddress and get ack from slave
  if (len != 0) {
    if(len > 1) {
      subAddr |= 0x80;
    }
  ...
  // Repeat ST to read data from slave
  if (i2cMstRepStrt(I2C_MST_RD_BIT) != mstAddrAckR)
  {
    len = 0;
  }
 ...
 */
uint8 HalI2CWrites(uint8 subAddr, uint8 len, uint8 *pBuf)
{
  // Write ST, slave address and get ack from slave
  if (i2cMstStrt(0) != mstAddrAckW)
  {
...
/**************************************************************************************************
 * @fn          HalI2CWrite (Legacy function)
 *
 * @brief       Write to the I2C bus as a Master.
 *
 * input parameters
