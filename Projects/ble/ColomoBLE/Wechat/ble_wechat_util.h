...
typedef void (*data_produce_func)(void *args, uint8 **r_data, uint32 *r_len);
typedef void (*data_free_func)(uint8 *data, uint32 len);
typedef int (*data_consume_func)(uint8 *data, uint32 len);
typedef void (*data_error_func)(int error_code);

typedef int32 (*data_button_handler_func)(void *p_wcs, uint8 pin_no);
typedef void (*data_main_process_func)(void *p_wcs);
typedef int32 (*data_init_func)(void);
